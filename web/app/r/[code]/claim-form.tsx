...
export function ClaimForm({ machine, banks }: { machine: Machine; banks: Bank[] }) {
  ...
  */
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/r/${machine.code}/done?receipt=${res.receiptCode}`);
    } catch (err) {
      // 입력값은 그대로 둔다. 다 지우고 다시 쓰게 하면 그 자리에서 포기한다.
      setError(err instanceof ApiError ? err.message : "문제가 생겼습니다. 다시 시도해주세요.");
      setSubmitting(false);
    }
  }

  const ready =
    issueType && amountNum > 0 && phone && bankCode && accountNo && holder && !photoMissing;
        ...
        */}
        {needsPhoto && (
          <div className="flex gap-2.5 rounded-lg bg-[var(--tone-warn-bg)] p-3.5 text-[var(--tone-warn-fg)]">
            ...
            <p className="text-sm">
              {krw(machine.reviewThresholdKrw)} 이상은 <b>사진이 필요합니다.</b> 확인을 위해
              사장님이 전화드릴 수 있습니다.
            </p>
          </div>
        )}
        ...
        {/* 환불 정보 */}
        <Field label="연락처" required hint="환불 확인을 위해 연락드릴 수 있습니다.">
          <div className="relative">
            <Phone
              size={18}
              className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--muted)]"
        ...
        <Field label="예금주" required hint="계좌에 등록된 이름과 같아야 합니다.">
          <Input
            type="text"
            autoComplete="name"
            value={holder}
