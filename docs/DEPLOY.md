...
**목록에 한국(춘천·서울)이 있으면 그걸 고른다. 없으면 도쿄 `ap-tokyo-1`.**

한국 리전은 프리티어 가입 화면에 **안 보이는 경우가 많다.** 영구히 막힌 건
아니고 그 시점에 재고가 없어서다. 목록에 없으면 기다릴 것 없이 일본으로 간다.

**도쿄를 고르는 이유:** ARM 인스턴스가 보통 몇 분 안에 뜬다는 보고가 꾸준히
있다. 오사카(`ap-osaka-1`)도 거리는 비슷하지만 프리티어 ARM 재고에 대한
자료를 찾지 못했다 — 모르는 쪽보다 아는 쪽을 고른다. 도쿄에서 계속 막히면
그때 오사카로 계정을 새로 만들어도 된다.

**레이턴시는 걱정하지 않아도 된다.** 서울 5ms, 도쿄 30~35ms 로 차이가 25ms
남짓이다. 눈 한 번 깜빡이는 데 100~400ms 가 걸린다. 게다가 손님 화면에서
시간을 잡아먹는 건 왕복 지연이 아니라 **지하 매장 LTE 의 전송 속도**이고,
그건 서버가 어디 있든 같다. 이 앱은 하루 수백 건 규모라 구조적으로 레이턴시가
문제 될 자리가 없다.

> 홈 리전이 일본이어도 나중에 다른 리전을 추가 구독할 수 있다. 다만
> **Always Free 자원은 홈 리전에서만** 만들어지므로, 무료로 쓰려면 결국
> 홈 리전에 두어야 한다.
