...
### 2.1 현금이냐 카드냐 — 환불 경로가 통째로 갈린다

위의 딥링크 송금은 **손님이 현금을 넣은 경우**의 이야기다. 카드로 결제했다면
보낼 돈이 없다. 사장님이 카드 단말기에서 **그 승인을 취소**하면 카드사가 손님
카드로 되돌려준다.

그래서 접수 폼의 첫 갈림길은 증상이 아니라 결제 수단이다.

| | 현금 | 카드 |
|---|---|---|
| 손님이 적는 것 | 은행·계좌번호·예금주 | 카드 뒷자리 4자리 (+ 대략의 결제 시각) |
| 사장님이 하는 일 | 딥링크로 송금 | 단말기에서 승인취소 |
| `payout_method` | `deeplink` / `manual` | `card_void` |
| 손님이 받는 안내 | "입력하신 계좌로 환불" | "결제를 취소, 카드사 사정으로 2~5일" |

설계상 중요한 지점 셋:

1. **카드 건에는 계좌를 받지 않는다.** 취소에 필요하지 않고, 받지 않으면 유출될
   것도 없다. 받아두고 안 쓰는 개인정보가 가장 나쁘다.
2. **전체 카드번호는 받지 않는다.** 단말기에서 거래를 찾는 데는 뒷자리 4자리와
   시각이면 충분하다. 전체 번호는 받는 순간 다루기 훨씬 무거운 정보가 된다.
3. **결제 수단과 처리 경로가 어긋나면 서버가 막는다.** 카드 건을 "계좌로 보냈다"로
   기록하면 승인은 살아 있는데 장부에는 환불이 끝난 것으로 남는다 — 그게 정확히
   이중 환불이 나는 자리다. `mark-paid`가 둘의 조합을 검증한다.

`payment_method` 의 세 번째 값 `unknown` 은 이 구분이 생기기 전에 들어온 기존
건이다. 전부 현금으로 간주하면 카드 건이 섞여 들어가 잘못된 통계가 된다.

> **리스크 규칙의 함정 하나:** 계좌 공유 규칙(같은 계좌에 여러 번호)은 계좌가
> 있는 건에만 적용해야 한다. 카드 건은 계좌가 빈 문자열이고, 그 해시는 서로
> 같다. 이 가드가 없으면 카드 신고가 세 건만 쌓여도 전부 "한 계좌를 공유한다"로
> 판정되어 검토 대상이 된다. `RiskInput.HasAccount` 가 그 가드다.
...
| `account_sharing` | 같은 계좌에 서로 다른 전화번호 3개 이상 (계좌가 있는 건만) | `needs_review` |
