...
- [x] **Step 2: 테스트 실패 확인**
- [x] **Step 3: 최소 구현** — 체크섬 가중치 `[1,3,7,1,3,7,1,3,5]`,
  9번째 자리의 기여분에 `*5/10` 을 더한 뒤 `(10 - sum%10) % 10`.
- [x] **Step 4: 테스트 통과 확인**
- [x] **Step 5: `bizverify.Verifier` 인터페이스 + `Checksum` 구현**
- [x] **Step 6: `arch_test.go` 통과 확인** (domain 격리)
- [x] **Step 7: 커밋** `feat(domain): 사업자등록번호 검증과 거래글 규칙`
...
- [x] **Step 1: 실패하는 테스트** (통합)
  ...
  - 매장당 일일 열람 수를 셀 수 있다
- [x] **Step 2~4: 구현과 통과 확인**
- [x] **Step 5: 커밋** `feat(store): 장터 글과 연락 기록`
...
- [x] **Step 1: 설정에 지역·사업자등록번호** — 입력하는 동안 체크섬 통과 여부를
  보여준다. "실제 존재하는 번호인지는 확인하지 않습니다"를 숨기지 않는다.
- [x] **Step 2: 장터 목록** — 지역·거래방식 필터. 전화 버튼은 누르기 전까지
  번호가 없다. 하단에 "claw-hub는 거래 당사자가 아닙니다" 고정
- [x] **Step 3: 재고에서 내놓기** — 재고 상세 시트에 `[장터에 내놓기]`.
  보유 수량과 내 원가를 같이 띄운다 (원가보다 싸게 내놓는 걸 막지는 않되,
  모르고 그러는 일은 없게)
- [x] **Step 4: 내 글 관리** — 내리기, 신고 확인
- [x] **Step 5: 빌드·타입체크 통과**
- [x] **Step 6: 커밋** `feat(web): 장터 화면`

---

### Task 29: 실제 기동 검증

- [x] **Step 1: 매장 두 곳을 만들어 서로의 글을 보는 시나리오** — 이게 이
  기능의 전부다. 한 매장만으로는 아무것도 검증되지 않는다
- [x] **Step 2: 번호 열람 기록이 실제로 쌓이는지 DB에서 확인**
- [x] **Step 3: 데스크톱·모바일 스크린샷, 콘솔 에러 0건**
- [x] **Step 4: `make check` + 통합 테스트 전체 통과**
- [x] **Step 5: 커밋** `docs: 장터 구현 반영`
...
| # | 증상 | 조치 |
|---|---|---|
| 1 | 연락처를 본 뒤 그 줄의 **신고 버튼이 사라졌다** | 연락처 블록이 버튼 줄을 통째로 대체하고 있었다. 오히려 전화해보고 말이 다른 걸 아는 순간이 신고할 일이 생기는 때다 — 둘을 나란히 뒀다 |

실제 매장 두 곳(서울·부산)으로 전 과정을 돌려 확인했다.

- 목록 HTML에 전화번호가 없다 → 버튼을 누르면 나온다 (브라우저에서 문자열로 확인)
- 자기 글의 연락처 요청은 403, 기록도 남지 않는다
- 열람 기록이 DB에 남는다: `해운대 인형나라 → 쿠로미 중형 30cm @ 09-24 00:46`
- 체크섬이 틀린 사업자번호(220-81-62518)는 400, 맞는 번호는 통과
- 필터: 전체 3건 / 서울 2건 / 부산 1건 / 교환 2건 / 판매 2건
  (판매·교환 글이 양쪽에 다 잡힌다)
- 모바일 가로 넘침 0px, 콘솔 에러 0건
