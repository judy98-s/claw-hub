...
- [x] **Step 2: 테스트 실패 확인** — `go test ./internal/domain/ -run Purchase` → 컴파일 실패
- [x] **Step 3: 최소 구현** — 반올림은 `(총액 + qty/2) / qty`. `NameKey`는 유니코드 공백
  전부 제거 + 소문자화. 왜 공백을 "접는" 게 아니라 "지우는"지 주석으로 남긴다.
- [x] **Step 4: 테스트 통과 확인**
- [x] **Step 5: `arch_test.go`가 여전히 통과하는지 확인** (domain 격리)
...
- [x] **Step 1: 실패하는 테스트** (통합, `-tags=integration`)
...
- [x] **Step 2: 테스트 실패 확인** — 마이그레이션 없음
- [x] **Step 3: 최소 구현** — 마이그레이션 + 쿼리. 재고는
  `purchases`를 `name_key`로 GROUP BY 하고 `inventory_adjustments` 합을 LEFT JOIN 해서 계산.
  가중평균 원가는 `SUM(unit_price*qty + shipping) / SUM(qty)`.
- [x] **Step 4: 테스트 통과 확인** (Postgres 필요)
- [x] **Step 5: 마이그레이션 두 번 실행해도 안전한지 확인** (기존 `TestMigrate_두번`)
- [x] **Step 6: 커밋** `feat(store): 사입 기록과 재고 집계`
...
- [x] **Step 1: 실패하는 테스트**
  ...
  - 자동완성이 같은 매장 것만 돌려준다
- [x] **Step 2: 테스트 실패 확인**
- [x] **Step 3: 최소 구현** — 영수증 업로드는 기존 `collectPhotos`/`writePhotos` 재사용.
  단, **보존 기간을 주지 않는다** (`ExpiresAt` 제로값) — 영수증은 지우지 않는다.
- [x] **Step 4: 테스트 통과 확인**
- [x] **Step 5: 사진 만료 워커가 영수증을 지우지 않는지 확인** — `ExpiredPhotos`는
  `claim_photos`만 보므로 영향 없다. 테스트로 못 박는다.
- [x] **Step 6: 커밋** `feat(api): 사입 등록과 재고 조회`
...
- [x] **Step 1: 메뉴에 "재고" 추가** — 접수함과 기계 사이
- [x] **Step 2: 목록 화면** — 빈 상태는 `[첫 사입 기록하기]` 버튼 하나 + 샘플 한 줄을
  흐리게. "아직 기록이 없습니다" 한 줄로 끝내지 않는다 (스펙 §7.2)
- [x] **Step 3: 콤보박스** — 기존 `Field`/`Input`과 라벨 연결이 되어야 한다
  (`__labelable`). 키보드 위/아래/Enter/Esc 동작
- [x] **Step 4: 사입 시트** — 실시간 개당 실단가, 지난번 단가
- [x] **Step 5: 실사 보정** — 품목 상세에서 "세어보니 n개"
- [x] **Step 6: `npx tsc --noEmit` + `npm run build` 통과 확인**
- [x] **Step 7: 커밋** `feat(web): 재고 장부 화면`

---

### Task 24: 실제 기동 검증과 문서

- [x] **Step 1: 서버·프론트 기동 후 브라우저로 전 과정 확인**
  사입 2건 등록 → 재고 목록 → 같은 이름 다시 등록해서 한 줄로 합쳐지는지 →
  실사 보정 → 데스크톱·모바일 스크린샷
- [x] **Step 2: 콘솔 에러 0건 확인** — 빌드가 통과해도 런타임에서 터지는 걸 여러 번 봤다
- [x] **Step 3: `make check` 전체 통과** (fmt + vet + 단위 + 통합)
- [x] **Step 4: 스펙 §3.1 갱신** — 구현된 내용과 어긋나는 부분 정정
- [x] **Step 5: 커밋** `docs: 재고 장부 구현 반영`

---

## QA에서 잡아 고친 것

계획을 세울 때 예상하지 못했고, 실제로 띄워 보고서야 드러난 것들이다.
전부 회귀 테스트를 붙였다.

| # | 증상 | 원인 | 조치 |
|---|---|---|---|
| 1 | 더블탭 재시도에 "지난번보다 싸게 샀습니다"가 떴다 | 기록되지 않은 건에 직전 단가를 붙였고, 그 직전 단가가 그 사이 들어간 다른 건이었다 | `existing`이면 비교값을 0으로 |
| 2 | "수량을(를) 숫자로 입력해주세요" | 조사를 템플릿으로 붙였다 | 라벨마다 맞는 조사를 직접 씀 |
| 3 | 한국 9월 1일 사입이 "9월 지출"에서 빠졌다 | 월 경계를 UTC 자정으로 잡았다 | `nowKST()` 도입, 홈의 "오늘"도 같이 수정 |
| 4 | 9/1 사입이 화면에 "8/31"로, "9월 인형값"이 "8월"로 | 브라우저가 KST 타임스탬프를 자기 시간대로 그렸다 | 모든 날짜 포맷을 `Asia/Seoul` 로 고정 |
| 5 | "9월 인형값 410,000원" vs "재고 자산 409,960원" — 40원 차이 | 반올림한 평균 원가를 수량에 곱했다 | 총 지출에서 남은 비율만큼 떼는 방식으로 |
| 6 | 한 대화상자에 "닫기"라는 이름의 컨트롤이 둘 | X 버튼과 고스트 버튼이 같은 이름 | 고스트 버튼을 "재고 목록으로"로 |
| 7 | 캐시 TTL 테스트가 간헐적으로 실패 | 여유가 15ms뿐이라 스케줄러 지터에 걸렸다 | 여유를 100ms로 (기존 테스트) |

## 셀프 리뷰에서 고친 것

| # | 문제 | 조치 |
|---|---|---|
| 1 | `URL.createObjectURL` 을 렌더에서 부르고 해제하지 않음 — 리렌더마다 파일 하나씩 메모리에 붙들림 | `PreviewImage` 컴포넌트로 묶어 effect 안에서 만들고 정리 시 revoke. 손님 신고 폼의 기존 누수도 같이 고침 |
| 2 | 실사 기록이 "읽고 나서 쓰는" 구조 — 두 사람이 동시에 세면 두 번째 차이가 틀린 기준에서 계산됨 | 현재 수량을 "마지막 실사값 + 그 뒤 입고"로 계산. 쓰기가 읽기에 의존하지 않아 경합이 사라짐. `-race` 동시 실사 테스트 추가 |
